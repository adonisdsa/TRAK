...
// import { persistToLocalStorage, restoreFromLocalStorage } from 'utils/local-storage-helper';
...
//      */
//     updateTree(actions = []) {
//         this.setState({ tree: this.kendoConvert(actions) }, this.storeStateKeysInStorage);
//     }
...
//             this.setState(
//                 state => ({
//                     expandedItems: state.expandedItems.filter(item => !item.ancestors.includes(directory)),
//                 }),
//                 this.updateTree
//             );
//         }
...
//             const included = !!expandedItems.find(({ directory }) => directory === key);

//             this.setState(
//                 {
//                     expandedItems: included ? expandedItems.filter(({ directory }) => directory !== key) : [...expandedItems, { directory: key, ancestors: item.ancestors }],
//                 },
//                 () => {
//                     if (included) {
...
//         return tree[0] ? tree[0].links.map(item => this.convertTreeToKendoTree(item, [], actions)) : [];
//     }
...
//         const { expandedItems } = this.state;
//         const opened = !!expandedItems.find(({ directory }) => directory === item.directory);
//         const childAncestors = !!item.directory ? [...parentAncestors, item.directory] : parentAncestors;

//         const node = {
//             text: item.directory ? item.directory : item.title,
//             opened,
//             items: Array.isArray(item.links) ? item.links.map(childItem => this.convertTreeToKendoTree(childItem, childAncestors, actions)) : [],
...
//                 <TreeView data={tree} textField="text" expandField="opened" itemRender={NavItem} onExpandChange={this.toggleItemExpansion} onItemClick={this.toggleItemExpansion} />
//             </div>
//         );
//     }
// }
